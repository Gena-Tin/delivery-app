...
const generateOrderCode = (length = 4) => {
...
shopRoutes.get("/shops", async (req, res) => {
    try {
...
shopRoutes.get("/goods", async (req, res) => {
    try {
...
shopRoutes.post("/orders", async (req, res) => {
    try {
        const { name, phone, email, address, goods, totalPrice, coordinates } = req.body;
        ...
        const result = await query(`INSERT INTO shop.orders (order_code, order_data) 
       VALUES ($1, $2) 
       RETURNING id, order_code, status, created_at`, [orderCode, JSON.stringify(orderData)]);
        res.status(201).json({
...
shopRoutes.get("/orders", async (req, res) => {
    try {
        const result = await query("SELECT * FROM shop.orders ORDER BY created_at DESC");
        res.json(result.rows);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
